...
function CommandPalette({ open, onClose, go }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => {
        Promise.all([
          window.rumboApi.policiesPicker(term),
          window.rumboApi.contactsPicker(term),
          window.rumboApi.claimsPicker(term),
        ])
          .then(([pr, cr, sr]) => {
            ...
            (pr.data || []).forEach(p =>
              its.push({
                ...
                kind: 'Póliza',
                label: p.client,
                meta: `${p.num} · ${p.insurer} · ${p.ramo}`,
                ramo: p.ramo,
                run: () => go('detail', { id: p.id }),
              }),
            );
            (cr.data || []).forEach(c =>
              its.push({
            ...
            (sr.data || []).forEach(s =>
              its.push({
                id: 's-' + s.id,
                kind: 'Siniestro',
                label: `${s.tipo} · ${s.client}`,
                meta: `${s.num} · ${s.status}`,
                icon: 'shield',
                run: () => window.rumboUI?.openClaim(s.id),
              }),
            );
            setItems(its);
          })
    ...
    >
      ...
      >
        ...
        <div className="scroll" style={{ maxHeight: 380, overflowY: 'auto', padding: 8 }}>
          {results.length === 0 && (
            <div style={{ padding: '36px 18px', textAlign: 'center', color: 'var(--ink-3)', fontSize: 13.5 }}>
          ...
          {results.map((r, i) => (
            ...
            >
              ...
              >
                {r.icon ? (
                  <Icon name={r.icon} size={16} stroke={2.1} />
                ) : r.ramo ? (
                  <Icon name={ramoIcon[r.ramo]} size={16} stroke={1.9} />
                ) : (
                  r.initials
                )}
              </span>
              ...
              <Pill tone={kindTone[r.kind]} style={{ fontSize: 10.5 }}>
                {r.kind}
              </Pill>
        ...
        >
          <span className="tick-row">
            <kbd className="font-mono" style={{ fontSize: 10.5 }}>
              ↑↓
            </kbd>{' '}
            navegar
          </span>
