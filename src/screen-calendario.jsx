...
function monthTitle(year, month) {
  const raw = new Intl.DateTimeFormat('es-AR', { month: 'long', year: 'numeric', timeZone: 'UTC' })
...
const NAV_BTN = {
  width: 34, height: 34, borderRadius: 'var(--radius-sm)', border: '1px solid var(--hair)',
...
function DayItemsList({ items, onEditEvent, onMutated, go }) {
  ...
  const del = (ev) => {
    ...
    setBusy(true);
    window.rumboApi.deleteCalendarEvent(ev.id)
      .then(() => { flash('Evento eliminado'); onMutated(); })
      .catch((e) => flash(e.message))
      .finally(() => setBusy(false));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, opacity: busy ? 0.6 : 1, transition: 'opacity .12s' }}>
        ...
        return (
          <div key={e.id} style={row}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: done ? 'var(--ink-3)' : 'var(--ink)', textDecoration: done ? 'line-through' : 'none' }}>
                {e.time ? `${e.time} · ` : ''}{e.title}
              </div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>
                {EVENT_KIND_LABELS[e.kind] || e.kind}{e.notes ? ` · ${e.notes}` : ''}
              </div>
            </div>
            <button onClick={() => onEditEvent(e)} title="Editar"
              style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--ink-3)', padding: '3px 7px', borderRadius: 6, border: '1px solid var(--hair)', background: 'var(--panel)', flexShrink: 0 }}>Editar</button>
            <button onClick={() => del(e)} title="Eliminar"
              style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--red-ink)', padding: '3px 7px', borderRadius: 6, border: '1px solid var(--hair)', background: 'var(--panel)', flexShrink: 0 }}>Borrar</button>
          </div>
        );
      })}
      ...
      {items.vencimientos.map((v) => (
        <div key={v.id} style={linkRow} onClick={() => go('detail', { id: v.id })}>
          <Pill tone={CAL_META.vencimiento.pill} dot>Vence</Pill>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.client}</div>
            <div style={{ fontSize: 11.5, color: 'var(--ink-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {v.ramo}{v.policyNumber ? ` · ${v.policyNumber}` : ''}{v.paymentMethod ? ` · ${v.paymentMethod}` : ''}
            </div>
          </div>
        </div>
      ))}
      ...
      {items.cuotas.map((c) => (
        <div key={c.id} style={linkRow} onClick={() => go('detail', { id: c.policyId })}>
          <Pill tone={CAL_META.cuota.pill} dot>Cuota</Pill>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.client}</div>
            <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>
              Cuota {c.number} · {ars(c.amount)}{c.paymentMethod ? ` · ${c.paymentMethod}` : ''}
            </div>
          </div>
        </div>
      ))}
      ...
      {items.siniestros.map((s) => (
        <div key={s.id} style={linkRow} onClick={() => go('siniestros')}>
          <Pill tone={CAL_META.siniestro.pill} dot>Siniestro</Pill>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.tipo} · {s.client}</div>
...
function EventDrawer({ open, onClose, defaultDate, event, onSaved }) {
  ...
  const submit = () => {
    ...
    setError(null);
    const p = editing
      ? window.rumboApi.updateCalendarEvent(event.id, data)
      : window.rumboApi.createCalendarEvent(data);
    p.then(() => {
  ...
  return (
    <Drawer open={open} onClose={onClose} width={520}
      eyebrow="Agenda" title={editing ? 'Editar evento' : 'Agendar evento'}
      footer={<>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div style={FORM_GRID}>
          <Field label="Tipo">
            <select value={kind} onChange={(e) => setKind(e.target.value)}
              ...
              {EVENT_KINDS.map((k) => <option key={k} value={k}>{EVENT_KIND_LABELS[k]}</option>)}
            </select>
          </Field>
        ...
        <Field label="Notas" hint="opcional" span={2}>
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} maxLength={2000}
            placeholder="Detalles del evento" style={{ ...inputStyle, resize: 'vertical', minHeight: 80 }} />
        </Field>
...
function ScreenCalendario({ go }) {
  ...
  const byDay = useMemo(() => {
    ...
    const get = (d) => {
      let e = map.get(d);
      if (!e) { e = { vencimientos: [], cuotas: [], siniestros: [], eventos: [] }; map.set(d, e); }
      return e;
    };
    if (data) {
  ...
  const goToday = () => { setYear(Number(hoy.slice(0, 4))); setMonth(Number(hoy.slice(5, 7))); setSelected(hoy); };
  const openNew = () => { setEditing(null); setEventOpen(true); };
  const openEdit = (e) => { setEditing(e); setEventOpen(true); };

  const segs = [{ id: 'mes', label: 'Mes' }, { id: 'agenda', label: 'Agenda' }];
  ...
  return (
    <div className="scroll rise" style={{ overflowY: 'auto', height: '100%', padding: pad }}>
      <div style={{ maxWidth: 1160, margin: '0 auto' }}>
        <PageHead eyebrow="Operación" tick={3} title="Calendario"
          sub="Tu mes de un vistazo: renovaciones, cuotas, siniestros y tu agenda, en un solo lugar."
          actions={<Btn variant="primary" icon="plus" onClick={openNew}>Agendar</Btn>} />
        ...
        {/* Leyenda + conteos */}
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '6px 16px', marginBottom: 18, fontSize: 12, color: 'var(--ink-3)' }}>
          {CAL_TYPES.map((t) => (
            <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 99, background: CAL_META[t].dot }} />
              {CAL_META[t].plural}
              <span className="font-mono tnum" style={{ color: 'var(--ink-2)' }}>{counts[t]}</span>
            </span>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 20, alignItems: 'start' }}>
            <MonthGrid grid={grid} byDay={byDay} hoy={hoy} selected={selected}
              onSelect={(cell) => { setSelected(cell.date); if (!cell.inMonth) { setYear(Number(cell.date.slice(0, 4))); setMonth(Number(cell.date.slice(5, 7))); } }} />
            <DayPanel date={selected} items={byDay.get(selected) || emptyDay}
              onNewEvent={openNew} onNewClaim={() => window.rumboUI && window.rumboUI.newSiniestro()}
              onEditEvent={openEdit} onMutated={refetch} go={go} />
          </div>
        ) : (
          <AgendaView grid={grid} byDay={byDay} hoy={hoy} selected={selected} onSelect={setSelected}
            onNewEvent={openNew} onEditEvent={openEdit} onMutated={refetch} go={go} />
        )}
      </div>

      <EventDrawer open={eventOpen} onClose={() => setEventOpen(false)}
...
function MonthGrid({ grid, byDay, hoy, selected, onSelect }) {
  ...
  const heads = ['lun', 'mar', 'mié', 'jue', 'vie', 'sáb', 'dom'];
  return (
    <div style={{ background: 'var(--panel)', border: '1px solid var(--hair)', borderRadius: 'var(--radius)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--hair)' }}>
        {heads.map((d) => (
          <div key={d} className="eyebrow" style={{ textAlign: 'center', padding: '8px 0', fontSize: 10 }}>{d}</div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
        {grid.map((cell, i) => {
          ...
          const chips = [
            ...
            ...items.cuotas.map((c) => ({ key: `c-${c.id}`, dot: CAL_META.cuota.dot, text: `Cuota ${c.number} ${c.client}` })),
            ...items.siniestros.map((s) => ({ key: `s-${s.id}`, dot: CAL_META.siniestro.dot, text: s.tipo })),
            ...items.eventos.map((e) => ({ key: `e-${e.id}`, dot: CAL_META.evento.dot, text: e.title, done: e.completedAt !== null })),
          ];
          ...
          return (
            <div key={cell.date} onClick={() => onSelect(cell)} style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                ...
                {chips.length > 3 && <span style={{ fontSize: 10, color: 'var(--ink-3)' }}>+{chips.length - 3} más</span>}
              </div>
            </div>
          );
        })}
  ...
  return (
    <Panel style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
        <div className="eyebrow" style={{ textTransform: 'none', fontSize: 12.5, color: 'var(--ink)', fontWeight: 600 }}>{dayTitle(date)}</div>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <Btn size="sm" variant="soft" icon="plus" onClick={onNewEvent}>Agendar</Btn>
        <Btn size="sm" variant="ghost" icon="shield" onClick={onNewClaim}>Siniestro</Btn>
      </div>
      {empty ? (
  ...
  if (days.length === 0) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: 'var(--ink-3)' }}>
        ...
        <div style={{ fontSize: 12.5, marginTop: 4, marginBottom: 16 }}>Ni vencimientos ni cuotas ni siniestros ni eventos este mes.</div>
        <Btn size="sm" variant="primary" icon="plus" onClick={onNewEvent} style={{ margin: '0 auto' }}>Agendar evento</Btn>
      </div>
    );
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {days.map((g) => {
        ...
        return (
          <div key={g.date} onClick={() => onSelect(g.date)} style={{
            display: 'flex', gap: 14, padding: 14, background: 'var(--panel)', border: '1px solid var(--hair)',
            borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-sm)',
            outline: g.date === selected ? '2px solid var(--orange)' : 'none', outlineOffset: -1,
          }}>
            <div style={{ width: 44, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <span className="eyebrow" style={{ fontSize: 10 }}>{weekdayShort(g.date)}</span>
              <span style={{ width: 32, height: 32, marginTop: 2, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 99, fontFamily: 'var(--font-mono)', fontSize: 15, fontWeight: 700, background: isToday ? 'var(--orange)' : 'transparent', color: isToday ? 'var(--paper)' : 'var(--ink)' }}>{g.day}</span>
            </div>
            <div style={{ minWidth: 0, flex: 1 }}>
              <DayItemsList items={items} onEditEvent={onEditEvent} onMutated={onMutated} go={go} />
            </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
        <Btn size="sm" variant="soft" icon="plus" onClick={onNewEvent}>Agendar</Btn>
        <Btn size="sm" variant="ghost" icon="shield" onClick={() => window.rumboUI && window.rumboUI.newSiniestro()}>Siniestro</Btn>
      </div>
    </div>
  );
}
...
function CalendarError({ error, onRetry }) {
  ...
  return (
    <div style={{ padding: 40, textAlign: 'center', color: 'var(--ink-3)' }}>
      ...
      {!auth && <Btn size="sm" variant="ghost" icon="refresh" onClick={onRetry} style={{ margin: '0 auto' }}>Reintentar</Btn>}
    </div>
  );
}
