...
function CalendarioHoyBanner({ go }) {
  ...
  return (
    ...
    >
      ...
      <Btn size="sm" variant="primary" iconRight="arrowRight" onClick={() => go('calendario')}>
        Ver calendario
      </Btn>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        {value}
        {accent && (
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-3)', fontFamily: 'var(--font-sans)' }}>
            {accent}
          </span>
        )}
      </div>
      {sub && (
        <div
          style={{ marginTop: 8, fontSize: 11.5, color: 'var(--ink-3)', display: 'flex', alignItems: 'center', gap: 5 }}
        >
          {sub}
        </div>
      )}
...
function CourseRow({ v, go, idx, isNext, isMobile }) {
  ...
  const dotColor = { red: 'var(--red)', orange: 'var(--orange)', amber: 'var(--amber)', emerald: 'var(--emerald)' }[
    tone
  ];
  if (isMobile) {
    return (
        ...
        <div style={{ width: 14, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <span
            style={{
              ...
              marginTop: 14,
              zIndex: 2,
              boxShadow: isNext ? '0 0 0 4px var(--orange-soft)' : 'none',
              flexShrink: 0,
            }}
          />
        </div>
        <div
          onClick={() => go('detail', { id: v.policyId })}
          style={{ flex: 1, padding: '12px 0', borderBottom: '1px solid var(--hair-2)', cursor: 'pointer' }}
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            ...
            <span className="font-mono tnum" style={{ fontSize: 12, color: 'var(--ink-2)' }}>
              {rumboFmt.ars(v.prima)}
            </span>
          </div>
          {isNext && (
            <Pill tone="orange" style={{ fontSize: 9.5, padding: '2px 7px', marginTop: 8 }}>
              PRÓXIMA RENOVACIÓN
            </Pill>
          )}
        </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {isNext && (
              <Pill tone="orange" style={{ fontSize: 9.5, padding: '2px 7px' }}>
                PRÓXIMA RENOVACIÓN
              </Pill>
            )}
          </div>
          <div className="font-mono" style={{ fontSize: 11.5, color: 'var(--ink-3)', marginTop: 2 }}>
            {v.insurer} · {v.ramo} · vence {v.date}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <div className="font-mono tnum" style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 6 }}>
            {rumboFmt.ars(v.prima)}
          </div>
        </div>
        <Btn
...
function MiniRow({ children, onClick, last }) {
  return (
    <div
      ...
      onMouseEnter={e => {
        if (onClick) e.currentTarget.style.opacity = 0.72;
      }}
      onMouseLeave={e => {
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        ...
        >
          <div>
            ...
            <h1
              ...
              style={{ fontSize: isMobile ? 28 : 40, lineHeight: 1.02, letterSpacing: '-0.03em', color: 'var(--ink)' }}
            >
              Hola, {rumboIdentity().name.split(/\s+/)[0]}.
            </h1>
            <p style={{ fontSize: isMobile ? 13.5 : 15, color: 'var(--ink-2)', marginTop: 8, maxWidth: 560 }}>
              Tu cartera está <strong style={{ color: 'var(--emerald-ink)' }}>en buen rumbo</strong>. Hay{' '}
              <strong style={{ color: 'var(--ink)' }}>
                {vence30} {vence30 === 1 ? 'renovación' : 'renovaciones'}
              </strong>{' '}
              en los próximos 30 días y <strong style={{ color: 'var(--red-ink)' }}>{ars(cuotasTotal)}</strong> en
              cuotas vencidas por recuperar.
            </p>
          </div>
          <div style={{ display: 'flex', gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            <Btn variant="primary" icon="calc" onClick={() => go('cotizador')} style={isMobile ? { flex: 1 } : {}}>
              Cotizar
            </Btn>
          </div>
        </div>
          ...
          {isMobile ? (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'var(--hair-2)' }}>
              ...
              >
                ...
                <div>
                  <div className="eyebrow" style={{ marginBottom: 5, whiteSpace: 'nowrap' }}>
                    Salud de cartera
                  </div>
            ...
            <div style={{ display: 'flex', alignItems: 'stretch', padding: '20px 0' }}>
              ...
              <InstrumentCell
                label="Prima anual gestionada"
                value={arsShort(annual)}
                sub={
                  <>
                    <Icon name="trending" size={13} style={{ color: 'var(--emerald)' }} />{' '}
                    {(BOOK.polizas ?? 0).toLocaleString('es-AR')} pólizas activas
                  </>
                }
              />
              <InstrumentCell
                label="Vencen en 30 días"
                value={String(vence30)}
                tone="var(--orange-ink)"
                sub={nextV ? <>Próximo en {nextV.days} días</> : 'Sin vencimientos'}
              />
              <InstrumentCell
                ...
                sub={
                  staleClaims > 0 ? (
                    <>
                      <span style={{ color: 'var(--red-ink)', fontWeight: 600 }}>{staleClaims}</span> sin movimiento
                    </>
                  ) : (
                    'Todos al día'
                  )
              ...
              <InstrumentCell
                ...
                value={arsShort(cuotasTotal)}
                tone="var(--red-ink)"
                sub={<>{CUOTAS.length} cuotas por cobrar</>}
              />
            </div>
          )}
        </Panel>
        ...
        <div
          className="rgrid"
          style={{ display: 'grid', gridTemplateColumns: '1.45fr 1fr', gap: isMobile ? 20 : 24, alignItems: 'start' }}
        >
          {/* LEFT — the course */}
          <Panel style={isMobile ? { padding: 16 } : {}}>
            <SectionHead
              ...
              action={
                !isMobile && (
                  <Btn size="sm" variant="bare" iconRight="arrowRight" onClick={() => go('vencimientos')}>
                    Ver todos
                  </Btn>
                )
              }
            ...
            >
              <span style={{ width: 17, display: 'flex', justifyContent: 'center' }}>
                <Icon name="flag" size={13} />
              </span>
              <span style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                Tu posición · hoy
              </span>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 20 : 24 }}>
            ...
            <Panel style={isMobile ? { padding: 16 } : {}}>
              <SectionHead
                label="Siniestros que pierden rumbo"
                action={
                  !isMobile && (
                    <Btn size="sm" variant="bare" iconRight="arrowRight" onClick={() => go('siniestros')}>
                      Ver
                    </Btn>
                  )
                }
              ...
              {SINIESTROS.map((s, i) => {
                ...
                return (
                  <MiniRow
                    key={s.id}
                    last={i === SINIESTROS.length - 1}
                    onClick={() => go('detail', { id: s.policyId })}
                  >
                    <span
                      style={{
                        width: 34,
            ...
            <Panel style={isMobile ? { padding: 16 } : {}}>
              ...
              >
                <div>
                  ...
                  <div className="font-display tnum" style={{ fontSize: isMobile ? 22 : 26, color: 'var(--red-ink)' }}>
                    {ars(cuotasTotal)}
                  </div>
                </div>
                <Btn size="sm" variant="primary" icon="whatsapp">
                  Reclamar
                </Btn>
              </div>
              {CUOTAS.map((c, i) => (
                <MiniRow key={c.id} last={i === CUOTAS.length - 1} onClick={() => go('detail', { id: c.policyId })}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="font-mono" style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 2 }}>
                      Cuota {c.cuota} · venció {c.due} · {Math.abs(c.days)} d
                    </div>
                  </div>
                  <div className="font-mono tnum" style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--red-ink)' }}>
                    {ars(c.amount)}
                  </div>
                </MiniRow>
              ))}
                    ...
                    {!isMobile && (
                      <span
                        style={{
                          fontSize: 10.5,
                          fontWeight: 600,
                          color: x.score === 'Alta' ? 'var(--emerald-ink)' : 'var(--ink-3)',
                        }}
                      >
                        {x.score} prob.
                      </span>
...
function MobileStat({ label, value, tone }) {
  return (
    <div style={{ background: 'var(--panel)', padding: '12px 16px' }}>
      <div className="eyebrow" style={{ marginBottom: 5, fontSize: 9.5 }}>
        {label}
      </div>
      <div className="font-display tnum" style={{ fontSize: 19, color: tone || 'var(--ink)' }}>
        {value}
      </div>
    </div>
  );
