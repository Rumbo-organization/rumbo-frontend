...
function DocumentsPanel({ docs, target, onChanged }) {
  ...
  const onFile = e => {
    ...
    window.rumboApi
      ...
      .then(() => {
        window.rumboUI?.toast?.('Documento subido');
        onChanged();
      })
  ...
  return (
    <Panel>
      <SectionHead
        label="Documentos"
        sub={docs.length ? `${docs.length} adjuntos` : 'Sin documentos'}
        action={
          <Btn size="sm" variant="soft" icon="plus" onClick={pick} style={{ opacity: busy ? 0.5 : 1 }}>
            {busy ? 'Subiendo…' : 'Subir'}
          </Btn>
        }
      />
      ...
      {docs.length === 0 ? (
        ...
        docs.map((d, i) => (
          <ExtraRow key={d.id} last={i === docs.length - 1}>
            ...
            <a
              href={window.rumboApi.documentUrl(d.id)}
              target="_blank"
              rel="noreferrer"
              style={{
                flex: 1,
        ...
        <ExtraRow key={r.id} last={i === risks.length - 1}>
          ...
          {r.patente && (
            <Pill tone="neutral" style={{ fontSize: 10.5 }}>
              <span className="font-mono">{r.patente}</span>
            </Pill>
          )}
        </ExtraRow>
      ))}
...
function EndosoDrawer({ open, onClose, policyId, onChanged }) {
  ...
  useEffect(() => {
    if (open) {
      setF({ number: '', type: 'endoso', issuedAt: '', premio: '', description: '' });
      setError(null);
    }
  }, [open]);
  ...
  const submit = () => {
    ...
    window.rumboApi
      .createEndorsement(policyId, data)
      .then(() => {
        window.rumboUI?.toast?.('Movimiento cargado');
        onChanged();
        onClose();
      })
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <Field label="Número" required span={1}>
          <TextInput
            value={f.number}
            onChange={v => set('number', v.replace(/[^0-9]/g, ''))}
            mono
            placeholder="0 = emisión"
          />
        </Field>
        ...
        <Field label="Fecha de emisión" span={1}>
          <input type="date" value={f.issuedAt} onChange={e => set('issuedAt', e.target.value)} style={inputStyle} />
        </Field>
        <Field label="Premio" span={1}>
          <TextInput
            value={f.premio}
            onChange={v => set('premio', v.replace(/[^0-9.]/g, ''))}
            mono
            prefix="$"
...
function EndososPanel({ endosos, policyId, onChanged }) {
  ...
  return (
    <Panel>
      ...
      <SectionHead
        label="Endosos y movimientos"
        sub={endosos.length ? `${endosos.length} en la póliza` : 'Sin movimientos cargados'}
        action={
          <Btn size="sm" variant="soft" icon="plus" onClick={() => setOpen(true)}>
            Cargar
          </Btn>
        }
      />
      {endosos.length === 0 ? (
        <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
          La póliza es la secuencia de sus movimientos. Cargalos a mano hasta que llegue la sync.
        </div>
      ) : (
        endosos.map((e, i) => (
          <ExtraRow key={e.id} last={i === endosos.length - 1}>
            <span className="font-mono tnum" style={{ width: 28, fontSize: 12, color: 'var(--ink-3)', flexShrink: 0 }}>
              #{e.number}
            </span>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="font-mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>
                {e.issuedAt || 'sin fecha'}
              </div>
            </div>
            {e.premio != null && (
              <span className="font-mono tnum" style={{ fontSize: 12.5, fontWeight: 600 }}>
                {ars(e.premio)}
              </span>
            )}
            <DelBtn
              onClick={() => runMut(window.rumboApi.deleteEndorsement(e.id), 'Movimiento eliminado', onChanged)}
            />
          </ExtraRow>
        ))
      )}
...
function PersonasPanel({ personas, policyId, onChanged }) {
  ...
  const submit = () => {
    ...
    setBusy(true);
    window.rumboApi
      .createPolicyParty(policyId, { contactId: who.id, role })
      .then(() => {
        window.rumboUI?.toast?.('Persona agregada');
        setAdding(false);
        setWho(null);
        onChanged();
      })
  ...
  return (
    <Panel>
      <SectionHead
        label="Personas de la póliza"
        sub={personas.length ? `${personas.length} además del titular` : 'Solo el titular'}
        action={
          <Btn size="sm" variant="soft" icon={adding ? 'x' : 'plus'} onClick={() => setAdding(a => !a)}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select
              ...
              style={{ ...inputStyle, appearance: 'none', cursor: 'pointer', flex: 1 }}
            >
              {PARTY_ROLES.map(([v, l]) => (
                <option key={v} value={v}>
                  {l}
            ...
            <Btn
              ...
              icon="check"
              onClick={submit}
              style={{ opacity: who && !busy ? 1 : 0.5, pointerEvents: who && !busy ? 'auto' : 'none' }}
            >
              Agregar
            </Btn>
          </div>
      ...
      {personas.length === 0 && !adding ? (
        <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
          Tomador, beneficiario, conductor y demás roles de la póliza.
        </div>
      ) : (
        personas.map((p, i) => (
...
function InstallmentsActions({ policyId, hasPlan, onChanged }) {
  ...
  const submit = () => {
    ...
    setError(null);
    window.rumboApi
      .generateInstallments(policyId, {
        count: Number(f.count),
        firstDueDate: f.firstDueDate,
        totalAmount: Number(f.totalAmount),
      })
      .then(() => {
        window.rumboUI?.toast?.('Plan de pagos generado');
        setOpen(false);
        onChanged();
      })
      .catch(e => setError(e.message))
  ...
  return (
    <>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>
            Reparte el total en cuotas mensuales desde la primera fecha.
          </div>
          <Field label="Cantidad de cuotas" required>
            <TextInput value={f.count} onChange={v => set('count', v.replace(/[^0-9]/g, ''))} mono />
          </Field>
          ...
          <Field label="Importe total" required>
            <TextInput
              value={f.totalAmount}
              onChange={v => set('totalAmount', v.replace(/[^0-9.]/g, ''))}
              mono
              prefix="$"
            />
          </Field>
      ...
      {hasPlan ? (
        <Btn size="sm" variant="ghost" onClick={redo}>
          Rehacer
        </Btn>
      ) : (
        <Btn size="sm" variant="soft" icon="plus" onClick={() => setOpen(true)}>
...
function RelacionesPanel({ relaciones, contactId, onChanged, go }) {
  ...
  const submit = () => {
    ...
    setBusy(true);
    window.rumboApi
      .createRelationship(contactId, { relatedContactId: who.id, type })
      .then(() => {
        window.rumboUI?.toast?.('Relación creada');
        setAdding(false);
        setWho(null);
        onChanged();
      })
  ...
  return (
    <Panel>
      <SectionHead
        label="Relaciones"
        sub={relaciones.length ? `${relaciones.length} vinculadas` : 'Sin relaciones'}
        action={
          <Btn size="sm" variant="soft" icon={adding ? 'x' : 'plus'} onClick={() => setAdding(a => !a)}>
            {adding ? 'Cerrar' : 'Vincular'}
          </Btn>
        }
      />
      {adding && (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            >
              {RELATION_TYPES.map(([v, l]) => (
                <option key={v} value={v}>{`Es ${l.toLowerCase()} de este asegurado`}</option>
              ))}
            </select>
            <Btn
              size="sm"
      ...
      {relaciones.length === 0 && !adding ? (
        <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
          Cónyuge, hijos, socios: el mapa familiar y comercial de la cartera.
        </div>
      ) : (
        relaciones.map((r, i) => (
          <ExtraRow key={r.id} last={i === relaciones.length - 1}>
            <div
              ...
              onClick={() => go && go('contacto', { id: r.otherContactId })}
            >
...
function DireccionesPanel({ direcciones, contactId, onChanged }) {
  ...
  const submit = () => {
    ...
    setBusy(true);
    window.rumboApi
      .createContactAddress(contactId, f)
      .then(() => {
        window.rumboUI?.toast?.('Dirección agregada');
        setAdding(false);
        setF({ label: '', street: '', number: '', city: '', province: '' });
        onChanged();
      })
      .catch(e => window.rumboUI?.toast?.(e.message))
      .finally(() => setBusy(false));
  ...
  return (
    <Panel>
      <SectionHead
        ...
        sub={direcciones.length ? `${direcciones.length} además del domicilio` : 'Solo el domicilio'}
        action={
          <Btn size="sm" variant="soft" icon={adding ? 'x' : 'plus'} onClick={() => setAdding(a => !a)}>
            {adding ? 'Cerrar' : 'Agregar'}
          </Btn>
      ...
      {adding && (
        ...
        >
          ...
          <Btn
            ...
            onClick={submit}
            style={{
              opacity: f.street.trim() && !busy ? 1 : 0.5,
              pointerEvents: f.street.trim() && !busy ? 'auto' : 'none',
            }}
          >
            Agregar
          </Btn>
      ...
      {direcciones.length === 0 && !adding ? (
        ...
        direcciones.map((d, i) => (
          <ExtraRow key={d.id} last={i === direcciones.length - 1}>
            ...
            <DelBtn
              onClick={() => runMut(window.rumboApi.deleteContactAddress(d.id), 'Dirección eliminada', onChanged)}
            />
          </ExtraRow>
        ))
      )}
...
function ResponsablesPanel({ responsables, contactId, onChanged }) {
  ...
  const submit = () => {
    ...
    setBusy(true);
    window.rumboApi
      .createAssignee(contactId, { userId, role })
      .then(() => {
        window.rumboUI?.toast?.('Responsable asignado');
        setAdding(false);
        onChanged();
      })
      .catch(e => window.rumboUI?.toast?.(e.message))
  ...
  return (
    <Panel>
      <SectionHead
        label="Responsables"
        sub={responsables.length ? `${responsables.length} asignados` : 'Sin responsables'}
        action={
          <Btn size="sm" variant="soft" icon={adding ? 'x' : 'plus'} onClick={() => setAdding(a => !a)}>
            {adding ? 'Cerrar' : 'Asignar'}
          </Btn>
        }
      />
      {adding && (
        ...
        >
          <select
            ...
            style={{ ...inputStyle, appearance: 'none', cursor: 'pointer', flex: 2, minWidth: 140 }}
          >
            {users.map(u => (
          ...
          <select
            value={role}
            onChange={e => setRole(e.target.value)}
            style={{ ...inputStyle, appearance: 'none', cursor: 'pointer', flex: 1, minWidth: 110 }}
          >
            {ASSIGNEE_ROLES.map(([v, l]) => (
              <option key={v} value={v}>
                {l}
              </option>
            ))}
          ...
          <Btn
            ...
            icon="check"
            onClick={submit}
            style={{ opacity: userId && !busy ? 1 : 0.5, pointerEvents: userId && !busy ? 'auto' : 'none' }}
          >
            Asignar
          </Btn>
        </div>
      ...
      {responsables.length === 0 && !adding ? (
        <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
          Quién atiende a este asegurado: comercial, cobranzas, siniestros.
        </div>
      ) : (
        responsables.map((r, i) => (
          <ExtraRow key={r.id} last={i === responsables.length - 1}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--ink-3)', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {r.email}
              </div>
            </div>
            <Pill tone="neutral" style={{ fontSize: 10.5 }}>
...
function ImportContactsDrawer({ open, onClose, onDone }) {
  ...
  const onFile = e => {
    ...
    file.text().then(text => {
      ...
      if (parsed.length < 2) {
        window.rumboUI?.toast?.('El archivo no tiene filas de datos.');
        return;
      }
  ...
  const buildRows = () =>
    rows.map(r => {
      ...
      return {
        ...
        addressProvince: get('addressProvince'),
        notes: get('notes'),
        status: 'asegurado',
      };
    });
        ...
        result ? (
          ...
          <>
            ...
            <Btn
              ...
              onClick={doImport}
              style={{
                opacity: headers && mapped > 0 && !busy ? 1 : 0.5,
                pointerEvents: headers && mapped > 0 && !busy ? 'auto' : 'none',
              }}
            >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {result ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13.5 }}>
            <div
              style={{ display: 'flex', alignItems: 'center', gap: 9, color: 'var(--emerald-ink)', fontWeight: 600 }}
            >
              <Icon name="check" size={17} /> {result.created.toLocaleString('es-AR')} asegurados importados
            </div>
            {result.skippedDuplicates > 0 && (
              <div style={{ color: 'var(--ink-2)' }}>
                {result.skippedDuplicates} salteados por DNI/CUIT ya existente.
              </div>
            )}
            {result.skippedInvalid > 0 && (
              <div style={{ color: 'var(--ink-2)' }}>
                {result.skippedInvalid} salteados por falta de nombre o razón social.
              </div>
            )}
          </div>
        ) : !headers ? (
          <>
            <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.55 }}>
              Subí un CSV con tus asegurados (separado por ; o ,). La primera fila debe tener los títulos de columna: se
              detectan solos y podés ajustar el mapeo antes de confirmar. Duplicados por DNI/CUIT se saltean.
            </div>
          ...
          <>
            <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>
              {rows.length.toLocaleString('es-AR')} filas · mapeá cada dato a su columna del archivo:
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {IMPORT_FIELDS.map(([field, label]) => (
                <div key={field} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, width: 92, flexShrink: 0, color: 'var(--ink-2)' }}>
                    {label}
                  </span>
                  <select
                    value={map[field] ?? ''}
                    onChange={e =>
                      setMap(m => ({ ...m, [field]: e.target.value === '' ? '' : Number(e.target.value) }))
                    }
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {buildRows()
                  ...
                  .map((r, i) => (
                    ...
                    >
                      {(r.kind === 'PERSONA_JURIDICA' ? r.legalName : `${r.lastName}, ${r.firstName}`) || '—'} ·{' '}
                      {r.dni || r.cuit || 'sin doc'} · {r.phone || r.email || 'sin contacto'} ·{' '}
                      {r.addressCity || 'sin ciudad'}
                    </div>
                  ))}
              </div>
            </div>
