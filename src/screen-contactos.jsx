      ...
      <>
        <Panel>
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 16 }}>
            <Avatar
              ...
              size={50}
              tone={
                detail.tags && (detail.tags.includes('Prospecto') || detail.tags.includes('Ex asegurado'))
                  ? 'neutral'
                  : 'orange'
              }
            />
            <div style={{ flex: 1, minWidth: 0 }}>
              <h2 className="font-display" style={{ fontSize: 21, letterSpacing: '-0.02em', lineHeight: 1.1 }}>
                {detail.name}
              </h2>
              <div style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 3 }}>
                {detail.kind} · desde {detail.since}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <Btn size="sm" variant="primary" icon="whatsapp" style={{ flex: 1 }}>
              WhatsApp
            </Btn>
            <Btn size="sm" variant="soft" icon="phone" style={{ flex: 1 }}>
              Llamar
            </Btn>
          </div>
          <Btn
            ...
            onClick={() => go('contacto', { id: detail.id })}
          >
            Ver ficha completa
          </Btn>
          <div
          ...
          >
            ...
            <MiniStat
              label="Siniestros"
              value={detail.stats.siniestros}
              tone={detail.stats.siniestros ? 'var(--amber-ink)' : undefined}
            />
        ...
        <Panel>
          <SectionHead
            ...
            action={
              <Btn size="sm" variant="bare" iconRight="arrowRight" onClick={() => go('polizas')}>
                Cartera
              </Btn>
            }
          />
            ...
            detail.polizas.map((p, i) => (
              <div
                ...
                style={{
                  ...
                  gap: 12,
                  padding: '11px 0',
                  borderBottom: i === detail.polizas.length - 1 ? 'none' : '1px solid var(--hair-2)',
                  cursor: 'pointer',
                }}
    ...
    >
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        <PageHead
          ...
          sub={
            <>
              ...
              {seg === 'todos' && !qDebounced ? 'en tu cartera' : 'en el filtro actual'}
            </>
          }
          actions={
            <>
              <Btn variant="ghost" icon="download" onClick={() => setImportOpen(true)}>
                Importar
              </Btn>
              <Btn
                variant="ghost"
              ...
              <Btn variant="primary" icon="plus" onClick={() => window.rumboUI?.newContacto()}>
                Nuevo asegurado
              </Btn>
            </>
          }
        ...
        <ImportContactsDrawer
          ...
          onDone={() => window.queryClient.invalidateQueries({ queryKey: ['contacts'] })}
        />
          ...
          <Panel pad={false} style={{ overflow: 'hidden' }}>
            {loading ? (
              ...
              <div style={{ padding: 50, textAlign: 'center', color: 'var(--red-ink)', fontSize: 13.5 }}>
                No pudimos cargar los asegurados.{' '}
                <button onClick={() => listQ.refetch()} style={{ color: 'var(--orange-ink)', fontWeight: 600 }}>
                  Reintentar
                </button>
              </div>
            ) : rows.length === 0 ? (
              <div style={{ padding: 50, textAlign: 'center', color: 'var(--ink-3)', fontSize: 14 }}>
                Sin asegurados que coincidan.
              </div>
            ) : (
              rows.map((c, i) => {
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {prospecto && (
                          <Pill tone="amber" style={{ fontSize: 9.5, padding: '1px 6px', flexShrink: 0 }}>
                            Prospecto
                          </Pill>
                        )}
                        {exasegurado && (
                          <Pill tone="neutral" style={{ fontSize: 9.5, padding: '1px 6px', flexShrink: 0 }}>
                            Ex asegurado
                          </Pill>
                        )}
                      </div>
                      ...
                      >
                        <Icon name="mapPin" size={12} style={{ flexShrink: 0 }} />
                        {c.city} · {c.kind}
                      </div>
                    </div>
          ...
          {!isMobile && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, position: 'sticky', top: 0 }}>
              {sel ? (
                ...
                <Panel>
                  <div style={{ padding: '30px 0', textAlign: 'center', color: 'var(--ink-3)', fontSize: 13.5 }}>
                    Elegí un asegurado para ver su resumen.
                  </div>
                </Panel>
              )}
            </div>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Btn
                size="sm"
                variant="ghost"
                onClick={() => setOffset(o => Math.max(0, o - CONTACTOS_LIMIT))}
              ...
              <Btn
                ...
                onClick={() => setOffset(o => o + CONTACTOS_LIMIT)}
      ...
      {isMobile && (
        <Drawer
          ...
          eyebrow="Resumen del asegurado"
          title={detail ? detail.name : 'Asegurado'}
        >
