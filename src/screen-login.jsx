...
function BrandWhite({ size = 30 }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
      ...
      />
      <span className="font-display" style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-0.03em', color: '#fff' }}>
        Rumbo
      </span>
    </div>
  );
...
function ScreenLogin({ onAuthed }) {
  ...
  const doEmail = async e => {
    ...
    try {
      if (mode === 'signup') {
        if (!terms) {
          setErr('Para crear la cuenta tenés que aceptar los Términos y la Política de privacidad.');
          setBusy(false);
          return;
        }
      ...
      if (mode === 'signup') {
        try {
          await window.rumboApi.acceptTerms();
        } catch {
          /* modal fallback */
        }
      }
  ...
  const doTotp = async e => {
    ...
    try {
      ...
      setErr('Código inválido o vencido.');
      setBusy(false);
    }
  };
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--paper)' }}>
      ...
      {!isMobile && (
        <div
          style={{
            ...
            minWidth: 0,
            background:
              'radial-gradient(130% 130% at 78% 8%, oklch(0.66 0.185 48) 0%, oklch(0.54 0.195 35) 46%, oklch(0.44 0.165 30) 100%)',
            display: 'flex',
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 500 }}>
              <h1
                ...
                style={{ fontSize: 46, lineHeight: 1.04, letterSpacing: '-0.03em', color: '#fff' }}
              >
                Marcá el rumbo
                <br />
                de tu cartera.
              ...
              <p style={{ fontSize: 16, color: 'oklch(1 0 0 / 0.82)', marginTop: 20, lineHeight: 1.55, maxWidth: 420 }}>
                El sistema para Productores Asesores de Seguros: asegurados, pólizas, vencimientos y siniestros en un
                solo lugar.
              </p>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {['Precios transparentes, sin letra chica', 'Sin permanencia', 'Tus datos, siempre exportables'].map(
                tx => (
                  <div
                    key={tx}
                    style={{
      ...
      <div
        ...
        style={{
          ...
          justifyContent: 'center',
          padding: isMobile ? '30px 20px 44px' : 40,
          overflowY: 'auto',
        }}
      >
            ...
            <form onSubmit={doForgot} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ fontSize: 13, textAlign: 'center' }}>
                <a
                  ...
                  style={{ color: 'var(--orange-ink)', fontWeight: 600, textDecoration: 'none' }}
                >
                  Volver a ingresar
                </a>
              </div>
            ...
            <form onSubmit={doEmail} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  minLength={8}
                  placeholder={signup ? 'Mínimo 8 caracteres' : '••••••••'}
                  style={inputStyle}
                />
                {!signup && (
                  <div style={{ textAlign: 'right', marginTop: 8 }}>
                    <a
                      ...
                      style={{ fontSize: 12.5, color: 'var(--orange-ink)', fontWeight: 600, textDecoration: 'none' }}
                    >
                      ¿La olvidaste?
                    </a>
                  </div>
