  ...
  return (
    <button
      ...
      style={{
        ...
        padding: 2,
        flexShrink: 0,
        background: on ? 'var(--emerald)' : 'var(--hair)',
...
function TwoFactorDrawer({ open, onClose, enabled, onChanged }) {
  ...
  const confirm = () => {
    ...
    setError(null);
    rumboAuth
      .twoFactorVerifyTotp(code.trim(), true)
      .then(() => {
        window.rumboUI?.toast?.('Verificación en dos pasos activada');
        onChanged(true);
        onClose();
      })
        ...
        step === 'password' ? (
          <>
            ...
            <Btn
              ...
              icon="check"
              onClick={start}
              style={{ opacity: password && !busy ? 1 : 0.5, pointerEvents: password && !busy ? 'auto' : 'none' }}
            >
          ...
          <>
            ...
            <Btn
              ...
              icon="check"
              onClick={confirm}
              style={{ opacity: code.trim() && !busy ? 1 : 0.5, pointerEvents: code.trim() && !busy ? 'auto' : 'none' }}
            >
              Verificar y activar
            </Btn>
          </>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {step === 'setup' && setup && (
          <>
            <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.5 }}>
              Cargá esta clave en tu app de autenticación (o abrí el link desde el teléfono) y después ingresá el código
              de 6 dígitos.
            </div>
            <Field label="Clave secreta (carga manual)">
              <div
                className="font-mono"
                style={{ ...inputStyle, userSelect: 'all', wordBreak: 'break-all', fontSize: 13 }}
              >
                {secret || '—'}
              </div>
            </Field>
            {setup.totpURI && (
              <a href={setup.totpURI} style={{ fontSize: 12.5, color: 'var(--orange-ink)', fontWeight: 600 }}>
...
function TemplatesEditor() {
  ...
  const save = () => {
    ...
    p.then(() => {
      window.rumboUI?.toast?.('Plantilla guardada');
      setEditing(null);
      invalidate();
    })
  ...
  const del = t => {
    window.rumboApi
      .deleteMessageTemplate(t.id)
      .then(() => {
        window.rumboUI?.toast?.('Plantilla eliminada');
        invalidate();
      })
      .catch(e => window.rumboUI?.toast?.(e.message));
  };
  ...
  return (
    <>
      <SectionHead
        ...
        sub="Tus textos propios para WhatsApp; se suman a los 4 prearmados."
        action={
          <Btn
            size="sm"
            variant="soft"
      ...
      {editing && (
        ...
        >
          <TextInput
            ...
            placeholder="Nombre (ej: Cobranza amable)"
          />
          <textarea
            value={f.body}
          ...
          >
            ...
            <Btn
              ...
              icon="check"
              onClick={save}
              style={{ opacity: f.name.trim() && f.body.trim() && !busy ? 1 : 0.5 }}
            >
              Guardar
            </Btn>
          </div>
      ...
      {rows.length === 0 && !editing ? (
        ...
        rows.map((t, i) => (
          ...
          >
            ...
            <Btn size="sm" variant="ghost" onClick={() => startEdit(t)}>
              Editar
            </Btn>
            <button
              title="Eliminar"
...
function ScreenConfiguracion({ go, dark, setDark }) {
  ...
  const [pas, setPas] = useState({
    cuit: org.cuit ?? '',
    matricula: org.matricula ?? '',
    fiscal: org.fiscalCondition ?? 'responsable_inscripto',
  });
  ...
  const savePas = () => {
    ...
    setPasBusy(true);
    window.rumboApi
      .updateOrgProfile({ cuit: pas.cuit, ssnMatricula: pas.matricula, fiscalCondition: pas.fiscal })
      .then(() => {
        window.rumboUI?.toast?.('Datos del PAS guardados');
  ...
  const doDeleteAccount = () => {
    ...
    window.rumboApi
      ...
      .catch(e => {
        window.rumboUI?.toast?.(e.message);
        setDeleting(false);
      });
  };
  ...
  const TEMPLATES = {
    renovacion: {
      label: 'Renovación',
      body: 'Hola {cliente}, te escribo de Méndez Seguros 👋 Tu póliza de {ramo} ({poliza}) vence el {vencimiento}. ¿Avanzamos con la renovación? Cualquier duda quedo a disposición.',
    },
    cobranza: {
      label: 'Cuota vencida',
      body: 'Hola {cliente}, te recuerdo que la cuota {cuota} de tu póliza {poliza} venció el {fecha} por {monto}. Podés abonarla por transferencia o link de pago. ¡Gracias!',
    },
    siniestro: {
      label: 'Siniestro',
      body: 'Hola {cliente}, recibimos tu denuncia de siniestro {siniestro}. Ya iniciamos la gestión con {aseguradora} y te mantengo al tanto de cada novedad.',
    },
    bienvenida: {
      label: 'Bienvenida',
      body: '¡Bienvenido/a, {cliente}! Soy tu productor/a de seguros. Ante cualquier consulta sobre tus coberturas, escribime directamente por acá.',
    },
  };
    ...
    >
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        <TwoFactorDrawer
          ...
          onChanged={on => {
            setTwofa(on);
            if (window.RUMBO_USER) window.RUMBO_USER.twoFactorEnabled = on;
          }}
        />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Panel>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--ink-3)', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {rumboIdentity().email || '—'}
                </div>
              </div>
            ...
            <SettingRow
              label="Plan"
              sub="Gratis durante la beta, sin permanencia."
              control={
          ...
          {window.RUMBO_DATA?.ME?.role === 'owner' && (
            <Panel>
              <SectionHead
                label="Datos del PAS"
                sub="Aparecen en tus comprobantes y comunicaciones cuando corresponde."
              />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <Field label="Razón social">
                  <div style={{ ...inputStyle, background: 'var(--panel-2)', color: 'var(--ink-2)' }}>
                    {org.name ?? '—'}
                  </div>
                </Field>
                <Field label="CUIT">
                  <TextInput
                    value={pas.cuit}
                    onChange={v => setPas(s => ({ ...s, cuit: v.replace(/[^0-9]/g, '') }))}
                    mono
                    placeholder="11 dígitos"
                  />
                </Field>
                <Field label="Matrícula SSN">
                  <TextInput value={pas.matricula} onChange={v => setPas(s => ({ ...s, matricula: v }))} mono />
                </Field>
                <Field label="Condición fiscal">
                  <select
                    value={pas.fiscal}
                    onChange={e => setPas(s => ({ ...s, fiscal: e.target.value }))}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
                <Btn variant="primary" size="sm" icon="check" onClick={savePas} style={{ opacity: pasBusy ? 0.5 : 1 }}>
                  Guardar datos
                </Btn>
              </div>
            </Panel>
          ...
          <Panel>
            ...
            <SettingRow
              ...
              sub={twofa ? 'Activada. Se pide un código TOTP al ingresar.' : 'Protegé el acceso con un código TOTP.'}
              control={<Toggle on={twofa} onClick={() => setTwofaOpen(true)} />}
            />
            <SettingRow
              label="Alertas de vencimientos y siniestros"
              sub="Avisos cuando una póliza o un caso pierde el rumbo."
              control={<Toggle on={alerts} onClick={() => setAlerts(!alerts)} />}
            />
            <SettingRow
              label="Plantillas de WhatsApp"
              sub="Mensajes prearmados para renovaciones y cobranzas."
              control={<Toggle on={wsp} onClick={() => setWsp(!wsp)} />}
            />
            <SettingRow
              label="Tema oscuro"
              sub="Cambia la apariencia de toda la app."
              control={<Toggle on={dark} onClick={() => setDark(!dark)} />}
              last
            />
          </Panel>
          ...
          <Panel>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <Btn
                variant="soft"
                icon="fileJson"
                onClick={() => window.open(window.rumboApi.accountExportUrl(), '_blank')}
              >
                Descargar todo (JSON)
              </Btn>
              <Btn
                variant="soft"
                icon="download"
                onClick={() => window.open(window.rumboApi.contactsExportUrl(), '_blank')}
              >
                Asegurados (CSV)
              </Btn>
              <Btn
          ...
          <div style={{ border: '1px solid var(--red)', borderRadius: 'var(--radius)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: 18, background: 'var(--panel)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>
                    Se borran de forma permanente tus pólizas, contactos y cotizaciones. Esta acción no se puede
                    deshacer.
                  </div>
                </div>
                {!confirmDel && (
                  <Btn
                    ...
                    variant="ghost"
                    icon="x"
                    style={{ color: 'var(--red-ink)', borderColor: 'var(--red)' }}
              ...
              {confirmDel && (
                ...
                >
                  <div style={{ fontSize: 12.5, color: 'var(--ink)', marginBottom: 10 }}>
                    Para confirmar, escribí <strong className="font-mono">ELIMINAR</strong> abajo.
                  </div>
                  <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <Btn
                      ...
                      style={{
                        background: 'var(--red)',
                        opacity: delText === 'ELIMINAR' ? 1 : 0.45,
                        pointerEvents: delText === 'ELIMINAR' ? 'auto' : 'none',
                      }}
                    >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 4px 0' }}>
            ...
            <Btn
              variant="ghost"
              icon="logout"
              style={{ color: 'var(--ink-2)', borderColor: 'var(--hair)' }}
