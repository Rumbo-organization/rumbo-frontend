        ...
        <PageHead
          ...
          sub={
            <>
              ...
              en pipeline · movelos de etapa y cerralos como ganados o perdidos
            </>
          }
          actions={
            <>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {loading && (
                    <span className="skel" style={{ display: 'block', width: '100%', height: 120, borderRadius: 11 }} />
                  )}
                  ...
                  {!loading &&
                    cards.map(p => (
                      ...
                      >
                        ...
                        {p.note && (
                          <div style={{ fontSize: 11.5, color: 'var(--ink-2)', lineHeight: 1.4, marginBottom: 11 }}>
                            {p.note}
                          </div>
                        )}
                        <div
                        ...
                        >
                          ...
                          >
                            <Icon name={ramoIcon[p.ramo] || 'shield'} size={13} />
                            {p.ramo}
                          </span>
                          <div style={{ display: 'flex', gap: 5 }}>
                            {next &&
                              actBtn(`→ ${next.label}`, `Pasar a ${next.label}`, () =>
                                move(p, next.id, `${p.name} pasó a ${next.label}`),
                              )}
                            {actBtn(
                              '✓',
                              'Cerrar ganado (pasa a asegurado)',
                              () => move(p, 'ganado', `${p.name} ganado: ya es asegurado`),
                              'ok',
                            )}
                            {actBtn(
                              '✕',
                              'Cerrar perdido (pasa a ex asegurado)',
                              () => move(p, 'perdido', `${p.name} marcado como perdido`),
                              'bad',
                            )}
                          </div>
                        </div>
