  ...
  return (
    <div
      ...
      style={{
        ...
        inset: 0,
        zIndex: 70,
        background: 'oklch(0.2 0.01 50 / 0.42)',
        backdropFilter: 'blur(3px)',
        display: 'flex',
        justifyContent: 'flex-end',
        animation: 'rumbo-fade .14s ease',
      }}
    >
      <div
        ...
        style={{
          ...
          animation: isMobile
            ? 'rumbo-sheet .24s cubic-bezier(0.16,1,0.3,1)'
            : 'rumbo-slide .26s cubic-bezier(0.16,1,0.3,1)',
        }}
      >
        <div
          style={{
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            <h2
              ...
              style={{ fontSize: isMobile ? 20 : 23, letterSpacing: '-0.02em', lineHeight: 1.05 }}
            >
              {title}
            </h2>
          </div>
        ...
        <div className="scroll" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '18px 16px' : '22px 24px' }}>
          {children}
        </div>
        {footer && (
          <div
...
function Field({ label, hint, children, span = 2, required }) {
  ...
  return (
    <div style={{ gridColumn: isMobile ? 'span 2' : `span ${span}` }}>
      ...
      >
        {label}
        {required && <span style={{ color: 'var(--orange-ink)' }}>*</span>}
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
      {prefix && (
        <span
          className="font-mono"
          style={{ position: 'absolute', left: 12, fontSize: 13, color: 'var(--ink-3)', pointerEvents: 'none' }}
        >
          {prefix}
        </span>
      )}
  ...
  return (
    <div style={{ position: 'relative' }}>
      <select
        ...
        style={{
          ...inputStyle,
          borderColor: foc ? 'var(--orange)' : 'var(--hair)',
          appearance: 'none',
          cursor: 'pointer',
          paddingRight: 34,
        }}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: 8 }}>
      {ramos.map(r => {
        ...
        return (
          ...
          >
            <Icon
              name={ramoIcon[r] || 'shield'}
              size={18}
              stroke={1.9}
              style={{ color: active ? 'var(--orange-ink)' : 'var(--ink-3)' }}
            />
            <span style={{ fontSize: 13, fontWeight: 600, color: active ? 'var(--orange-ink)' : 'var(--ink-2)' }}>
              {r}
            </span>
          </button>
        );
...
const WSP_TEMPLATES = [
  [
    'saludo',
    'Saludo',
    'Hola {nombre}, ¿cómo estás? Soy tu productor/a de seguros. Quedo a disposición por cualquier consulta sobre tus coberturas.',
  ],
  [
    'vencimiento',
    'Vencimiento',
    'Hola {nombre}, te escribo porque se acerca el vencimiento de tu póliza. ¿Coordinamos la renovación?',
  ],
  [
    'pago',
    'Cobranza',
    'Hola {nombre}, te recuerdo que tenés una cuota pendiente de pago. Cualquier duda me avisás por acá.',
  ],
  [
    'siniestro',
    'Siniestro',
    'Hola {nombre}, recibimos tu denuncia de siniestro. Ya iniciamos la gestión con la aseguradora y te mantengo al tanto.',
  ],
];
...
function WhatsAppDialog({ open, onClose, contact, policyId, onLogged }) {
  ...
  useEffect(() => {
    if (open) {
      setTpl('saludo');
      setBody(WSP_TEMPLATES[0][2].replace('{nombre}', firstName));
      setError(null);
    }
  }, [open, contact]);
  ...
  const pickTpl = k => {
    ...
    if (t) {
      setBody(t[2].replace('{nombre}', firstName));
      return;
    }
  ...
  return (
    <Drawer
      ...
      onClose={onClose}
      eyebrow="WhatsApp"
      title={`Escribirle a ${contact.name}`}
      width={520}
      footer={
        <>
          <Btn variant="ghost" onClick={() => log(false)} style={{ opacity: busy ? 0.5 : 1 }}>
            Solo registrar
          </Btn>
          <Btn
            variant="primary"
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {!phone && (
          ...
          >
            Este asegurado no tiene teléfono cargado: solo se puede registrar la comunicación.
          </div>
        )}
        ...
        <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>
          Se abre WhatsApp con el texto listo y la comunicación queda registrada en la ficha del asegurado.
        </div>
    ...
    <div ref={boxRef} style={{ position: 'relative' }}>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        <Icon
          name="search"
          size={14}
          style={{ position: 'absolute', left: 11, color: 'var(--ink-3)', pointerEvents: 'none' }}
        />
        <input
          value={q}
          placeholder={placeholder}
      ...
      {open && (
        ...
        >
          {loading && rows.length === 0 && (
            <div style={{ padding: '10px 12px', fontSize: 12.5, color: 'var(--ink-3)' }}>Buscando…</div>
          )}
          {!loading && rows.length === 0 && (
...
const CLAIM_TIPOS = [
  ...
  ['resp_civil', 'Responsabilidad civil'],
  ['otros', 'Otro'],
];
const EMPTY_SINIESTRO = {
  policyId: '',
  tipo: 'choque',
  occurredAt: '',
  reportedBy: '',
...
function NuevoSiniestroForm({ open, onClose }) {
  ...
  const submit = () => {
    ...
    window.rumboApi
      .createClaim(data)
      .then(() => {
        window.rumboUI?.toast?.('Siniestro cargado');
...
function NuevoContactoForm({ open, onClose }) {
  ...
  const submit = () => {
    ...
    window.rumboApi
      .createContact(data)
      .then(() => {
        window.rumboUI?.toast?.('Prospecto creado');
        if (window.rumboRefresh) window.rumboRefresh();
        onClose();
      })
      .catch(e => setError(e.message))
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: 10,
                borderRadius: 9,
                border: `1px solid ${f.kind === v ? 'var(--orange)' : 'var(--hair)'}`,
                background: f.kind === v ? 'var(--orange-soft)' : 'var(--panel)',
                color: f.kind === v ? 'var(--orange-ink)' : 'var(--ink-2)',
                fontWeight: 600,
                fontSize: 13,
              }}
            >
        ...
        <div style={useFormGrid()}>
          {juridica ? (
            <Field label="Razón social" required span={2}>
              <TextInput
                value={f.legalName}
            ...
            <>
              <Field label="Nombre">
                <TextInput value={f.firstName} onChange={v => set('firstName', v)} />
              </Field>
              <Field label="Apellido">
                <TextInput value={f.lastName} onChange={v => set('lastName', v)} />
              </Field>
            </>
          )}
          {juridica ? (
            <Field label="CUIT">
              <TextInput
                value={f.cuit}
                onChange={v => set('cuit', v.replace(/[^0-9]/g, ''))}
                mono
            ...
            <Field label="DNI">
              <TextInput
                value={f.dni}
                onChange={v => set('dni', v.replace(/[^0-9]/g, ''))}
                mono
                placeholder="opcional"
              />
            </Field>
          ...
          <Field label="Ciudad">
            <TextInput value={f.city} onChange={v => set('city', v)} placeholder="opcional" />
          </Field>
          <Field label="Teléfono" span={2}>
            <TextInput value={f.phone} onChange={v => set('phone', v)} mono placeholder="+54 9 ..." />
          </Field>
        ...
        >
          ...
          <span>
            Nace como <strong style={{ color: 'var(--orange-ink)' }}>prospecto</strong>. Pasa a asegurado cuando
            importás su primera póliza.
          </span>
        </div>
...
function fromContact(contact) {
  const c = contact || {};
  const methods =
    Array.isArray(c.contactMethods) && c.contactMethods.length
...
function EditContactoForm({ open, onClose, contact, onSaved }) {
  ...
  const setMethod = (i, k, v) =>
    setF(s => ({ ...s, methods: s.methods.map((m, j) => (j === i ? { ...m, [k]: v } : m)) }));
  const setPrimary = i => setF(s => ({ ...s, methods: s.methods.map((m, j) => ({ ...m, primary: j === i })) }));
  const addMethod = () =>
    setF(s => ({ ...s, methods: [...s.methods, { type: 'celular', value: '', primary: s.methods.length === 0 }] }));
  ...
  const submit = () => {
    ...
    const data = {
      ...
      contactMethods: f.methods
        .filter(m => m.value.trim())
        .map(m => ({ type: m.type, value: m.value.trim(), primary: !!m.primary })),
    };
    if (juridica) {
      data.legalName = f.legalName;
      data.cuit = f.cuit;
    } else {
      data.firstName = f.firstName;
      data.lastName = f.lastName;
    ...
    window.rumboApi
      ...
      .then(() => {
        window.rumboUI?.toast?.('Asegurado actualizado');
        if (window.rumboRefresh) window.rumboRefresh();
        onSaved && onSaved();
        onClose();
      })
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={useFormGrid()}>
          {juridica ? (
            <Field label="Razón social" required span={2}>
              <TextInput value={f.legalName} onChange={v => set('legalName', v)} />
            </Field>
          ) : (
            <>
              <Field label="Nombre">
        ...
        <div>
          ...
          <div style={useFormGrid()}>
            <Field label="Calle" span={1}>
              <TextInput value={f.addressStreet} onChange={v => set('addressStreet', v)} placeholder="Av. Colón" />
            </Field>
            <Field label="Número" span={1}>
              <TextInput value={f.addressNumber} onChange={v => set('addressNumber', v)} mono />
            </Field>
            <Field label="Piso" span={1}>
              <TextInput value={f.addressFloor} onChange={v => set('addressFloor', v)} mono placeholder="opcional" />
            </Field>
            <Field label="Depto" span={1}>
              <TextInput
                value={f.addressApartment}
                onChange={v => set('addressApartment', v)}
                mono
                placeholder="opcional"
              />
            </Field>
            <Field label="Ciudad" span={1}>
              <TextInput value={f.addressCity} onChange={v => set('addressCity', v)} />
            </Field>
            <Field label="Provincia" span={1}>
              <TextInput value={f.addressProvince} onChange={v => set('addressProvince', v)} />
            </Field>
            <Field label="Código postal" span={2}>
              <TextInput
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <Btn size="sm" variant="ghost" icon="plus" onClick={addMethod}>
              Agregar
            </Btn>
          </div>
          {f.methods.length === 0 ? (
            <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>
              Sin medios. Agregá teléfono, celular, WhatsApp o email.
            </div>
          ) : (
              ...
              {f.methods.map((m, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <TextInput
                      value={m.value}
                      onChange={v => setMethod(i, 'value', v)}
                      mono
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: m.primary ? 'var(--orange-ink)' : 'var(--ink-3)',
                      cursor: 'pointer',
                    }}
                  >
                    Principal
