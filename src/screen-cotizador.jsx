...
function ScreenCotizador({ go, onEmit }) {
  ...
  const insurerFactor = {
    'San Cristóbal': 0.94,
    'Federación Patronal': 1.0,
    'Sancor Seguros': 1.08,
    'La Segunda': 0.97,
    'Mercantil Andina': 1.12,
    Rivadavia: 0.9,
  };
  const coverPerks = {
  ...
  if (quotes.length) {
    quotes[0].best = true;
    quotes[1] && (quotes[1].recommended = true);
  }

  const Stepper = () => {
    const steps = [
      { n: 1, l: 'Datos del riesgo' },
      { n: 2, l: 'Comparar' },
      { n: 3, l: 'Emitir' },
    ];
    return (
            ...
            >
              <span
                style={{
                  ...
                  border: step >= s.n ? 'none' : '1px solid var(--hair)',
                  transition: 'all .2s',
                }}
              >
                {step > s.n ? <Icon name="check" size={14} stroke={3} /> : s.n}
              </span>
              {(!isMobile || step === s.n) && (
                <span
                  style={{
            ...
            {i < steps.length - 1 && (
              <div
                style={{
                  ...
                  background: step > s.n ? 'var(--emerald)' : 'var(--hair)',
                  borderRadius: 2,
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1080, margin: '0 auto' }}>
        ...
        <Panel style={{ marginBottom: 24, padding: isMobile ? '14px 16px' : '22px 26px' }}>
          <Stepper />
        </Panel>
          ...
          >
            <Panel>
              ...
              <div className="rgrid" style={FORM_GRID}>
                <Field label="Cliente / prospecto" required span={2}>
                  <SearchPicker
                    value={f.cliente ? { id: 'sel', name: f.cliente } : null}
                ...
                {ramo === 'Automotor' ? (
                  <>
                    ...
                    <Field label="Código postal">
                      <TextInput
                        value={f.cp}
                        onChange={v => set('cp', v.replace(/[^0-9]/g, ''))}
                        mono
                        placeholder="5000"
                      />
                    </Field>
                    <Field label="Suma asegurada">
                      <TextInput
                        value={f.suma}
                        onChange={v => set('suma', v.replace(/[^0-9]/g, ''))}
                        mono
                        prefix="$"
                        placeholder="0"
                      />
                  ...
                  <>
                    <Field label="Detalle del riesgo" span={2}>
                      <TextInput
                        ...
                        placeholder="Domicilio, rubro o datos del riesgo"
                      />
                    </Field>
                    <Field label="Código postal">
                      <TextInput
            ...
            <Panel style={{ position: 'sticky', top: 0 }}>
              ...
              <p style={{ fontSize: 12.5, color: 'var(--ink-2)', lineHeight: 1.5, marginBottom: 18 }}>
                Rumbo consulta tus aseguradoras en paralelo y te ordena las primas de menor a mayor. Vos elegís el
                rumbo.
              </p>
              <Btn variant="primary" icon="compass" onClick={() => setStep(2)} style={{ width: '100%' }}>
                Cotizar en {INSURERS.length} aseguradoras
              </Btn>
            </Panel>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <SectionHead
                label={`${quotes.length} cotizaciones · ${ramo}`}
                sub={f.marca || f.cliente || 'Riesgo a cotizar'}
              />
              <Btn
                size="sm"
                variant="ghost"
              ...
              {quotes.map(qt => (
                <div
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${qt.best ? 'var(--emerald)' : 'var(--hair)'}`,
                ...
                >
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <div className="font-display tnum" style={{ fontSize: 30, lineHeight: 1, color: 'var(--ink)' }}>
                      {ars(qt.monthly)}
                      <span
                        style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-3)', fontFamily: 'var(--font-sans)' }}
                      >
                        {' '}
                        /mes
                      </span>
                    </div>
                    <div className="font-mono tnum" style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 4 }}>
                      {ars(qt.annual)} anual
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 18 }}>
                    {qt.perks.map(p => (
                      <div
                        key={p}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--ink-2)' }}
                      >
                        <Icon name="check" size={14} stroke={2.4} style={{ color: 'var(--emerald)', flexShrink: 0 }} />
                        {p}
                      </div>
                    ))}
                  </div>
                  <Btn
                    variant={qt.best ? 'primary' : 'soft'}
                    icon="arrowRight"
                    onClick={() => {
                      setPicked(qt);
                      setStep(3);
        ...
        {step === 3 && picked && (
          <div style={{ maxWidth: 620, margin: '0 auto' }}>
            <Panel style={{ padding: 28, textAlign: 'center' }}>
              ...
              <h2 className="font-display" style={{ fontSize: 24, letterSpacing: '-0.02em', marginBottom: 6 }}>
                Confirmar emisión
              </h2>
              <p style={{ fontSize: 13.5, color: 'var(--ink-2)', marginBottom: 22 }}>
                Revisá los datos y emití la póliza en {picked.insurer}.
              </p>
  ...
  return (
    <div
      style={{
        ...
        background: strong ? 'var(--panel-2)' : 'transparent',
      }}
    >
      <span style={{ fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 600 }}>{label}</span>
      <span
        className={typeof value === 'string' && value.startsWith('$') ? 'font-mono tnum' : ''}
        style={{
          fontSize: strong ? 15 : 13.5,
          fontWeight: strong ? 700 : 500,
          color: strong ? 'var(--orange-ink)' : 'var(--ink)',
        }}
      >
        {value}
      </span>
