...
function NuevaCotizacionDrawer({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (open) {
      setWho(null);
      setF({ ramo: 'automotor', reference: '', marca: '', modelo: '', anio: '', notes: '' });
      setError(null);
    }
  }, [open]);
  ...
  const submit = () => {
    ...
    window.rumboApi
      .createQuote({
        contactId: who ? who.id : undefined,
        ramo: f.ramo,
        reference: f.reference,
        vehicleMarca: f.marca,
        vehicleModelo: f.modelo,
        vehicleAnio: f.anio,
        notes: f.notes,
      })
      .then(d => {
        window.rumboUI?.toast?.('Cotización creada');
        onCreated(d.id);
        onClose();
      })
    ...
    <Drawer
      ...
      footer={
        <>
          ...
          <Btn variant="primary" icon="check" onClick={submit} style={{ opacity: busy ? 0.5 : 1 }}>
            Crear
          </Btn>
        </>
      }
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <Field label="Referencia" span={1} hint="si no hay asegurado">
            <TextInput value={f.reference} onChange={v => set('reference', v)} placeholder="Ej: Sr. Gómez" />
          </Field>
          <Field label="Marca" span={1}>
            <TextInput value={f.marca} onChange={v => set('marca', v)} placeholder="opcional" />
          </Field>
          <Field label="Modelo" span={1}>
            <TextInput value={f.modelo} onChange={v => set('modelo', v)} placeholder="opcional" />
          </Field>
          <Field label="Año" span={1}>
            <TextInput
              value={f.anio}
              onChange={v => set('anio', v.replace(/[^0-9]/g, ''))}
              mono
...
function QuoteDetailDrawer({ open, onClose, quoteId, onChanged }) {
  ...
  const addItem = () => {
    ...
    window.rumboApi
      ...
      .then(() => {
        window.rumboUI?.toast?.('Opción cargada');
        setItem(s => ({ ...s, cuota: '', sumaAsegurada: '' }));
        refetch();
      })
  ...
  const delQuote = () => {
    ...
    window.rumboApi
      .deleteQuote(quoteId)
      .then(() => {
        window.rumboUI?.toast?.('Cotización eliminada');
        onChanged();
        onClose();
      })
      .catch(e => window.rumboUI?.toast?.(e.message));
  ...
  if (!q)
    return open ? (
      <Drawer open onClose={onClose} eyebrow="Cotización" title="Cargando…" width={640}>
        <span className="skel" style={{ display: 'block', width: '100%', height: 200, borderRadius: 10 }} />
      </Drawer>
    ) : null;

  const best = q.items.reduce(
    (acc, it) => (it.cuota != null && (acc == null || it.cuota < acc.cuota) ? it : acc),
    null,
  );
  ...
  return (
    <Drawer
      open={open}
      onClose={onClose}
      eyebrow={`Cotización ${q.num}`}
      title={q.client}
      width={680}
      footer={
        <>
          <Btn variant="ghost" onClick={delQuote} style={{ color: 'var(--red-ink)' }}>
            Eliminar
          </Btn>
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 12.5, color: 'var(--ink-2)' }}>
          ...
          {q.vehicle && (
            <Pill tone="neutral">
              <span className="font-mono">{q.vehicle}</span>
            </Pill>
          )}
        ...
        {adding && (
          ...
          >
            <select
              ...
              onChange={e => setItem(s => ({ ...s, insurerId: e.target.value }))}
              style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }}
            >
              {insurersList.map(i => (
            ...
            <select
              value={item.coverage}
              onChange={e => setItem(s => ({ ...s, coverage: e.target.value }))}
              style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }}
            >
              {COVERAGES.map(([v, l]) => (
                <option key={v} value={v}>
            ...
            <TextInput
              value={item.cuota}
              onChange={v => setItem(s => ({ ...s, cuota: v.replace(/[^0-9.]/g, '') }))}
              mono
              prefix="$"
            ...
            <TextInput
              ...
              onChange={v => setItem(s => ({ ...s, sumaAsegurada: v.replace(/[^0-9.]/g, '') }))}
              mono
              prefix="$"
            ...
            <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end' }}>
              <Btn
                ...
                icon="check"
                onClick={addItem}
                style={{ opacity: item.insurerId && (item.cuota || item.sumaAsegurada) && !busy ? 1 : 0.5 }}
              >
                Agregar
              </Btn>
            </div>
        ...
        {q.items.length === 0 && !adding && (
          <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
            Cargá las opciones que te pasó cada aseguradora para compararlas.
          </div>
        )}
          ...
          q.items.map(it => (
            <div
              ...
              style={{
                ...
                padding: '10px 12px',
                borderRadius: 10,
                border: `1px solid ${best && it.id === best.id ? 'var(--emerald)' : 'var(--hair)'}`,
                background: best && it.id === best.id ? 'var(--emerald-soft)' : 'var(--panel)',
              }}
            >
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13.5, fontWeight: 600 }}>
                  {it.insurer}
                  {best && it.id === best.id && (
                    <span style={{ fontSize: 10.5, color: 'var(--emerald-ink)', marginLeft: 8, fontWeight: 700 }}>
                      MEJOR PRECIO
                    </span>
                  )}
                </div>
                <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>
                  {it.coverageLabel || 'Sin cobertura normalizada'}
                  {it.sumaAsegurada ? ` · SA ${ars(it.sumaAsegurada)}` : ''}
                </div>
              </div>
              {it.cuota != null && (
                <span className="font-mono tnum" style={{ fontSize: 14, fontWeight: 700 }}>
                  {ars(it.cuota)}
                  <span style={{ fontSize: 11, color: 'var(--ink-3)', fontWeight: 400 }}>/mes</span>
                </span>
              )}
              <button
                title="Quitar"
        ...
        {view === 'matriz' &&
          (covs.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--ink-3)' }}>
              La matriz necesita opciones con cobertura normalizada.
            </div>
          ) : (
            <div style={{ overflowX: 'auto' }} className="scroll">
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: 'left', padding: '6px 8px' }} className="eyebrow">
                      Cobertura
                    </th>
                    {insCols.map(ins => (
                      <th key={ins} style={{ textAlign: 'right', padding: '6px 8px' }} className="eyebrow">
                        {ins}
                      </th>
                    ))}
                  </tr>
                        ...
                        return (
                          <td
                            ...
                            style={{
                              ...
                              color: isBest ? 'var(--emerald-ink)' : 'var(--ink)',
                            }}
                          >
    ...
    >
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <PageHead
          ...
          sub={
            <>
              ...
              cotizaciones · compará coberturas y precios entre aseguradoras
            </>
          }
          actions={
            <>
              <Btn variant="ghost" icon="calc" onClick={() => go('cotizador')}>
                Estimador
              </Btn>
              <Btn variant="primary" icon="plus" onClick={() => setNewOpen(true)}>
                Nueva cotización
              </Btn>
            </>
          }
        ...
        <NuevaCotizacionDrawer
          ...
          onCreated={id => {
            window.queryClient.invalidateQueries({ queryKey: ['cotizaciones'] });
            setOpenId(id);
          }}
        />
        <QuoteDetailDrawer
